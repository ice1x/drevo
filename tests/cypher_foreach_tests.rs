...
use drevo::cypher::executor::{execute_on_engine as execute, ExecError, ExecStats, Value};
...
fn exec(source: &str, drevo: &NativeGraph) -> ExecStats {
...
fn exec_err(source: &str, drevo: &NativeGraph) -> ExecError {
    let q = parse(source).expect("parse");
