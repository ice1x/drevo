...
fn neighbors_returns_depth_1() {
    ...
    let result = db.neighbors(a.id, Direction::Outgoing, None);
    assert_eq!(result.len(), 1);
    assert_eq!(result[0].id, b.id);
}
...
fn neighbors_with_edge_kind_filter() {
    ...
    let result = db.neighbors(a.id, Direction::Outgoing, Some("tagged_with"));
    assert_eq!(result.len(), 1);
...
fn scenario_erp_order_graph() {
    ...
    // All order neighbors
    let result = db.neighbors(order.id, Direction::Outgoing, None);
    assert_eq!(result.len(), 3); // product1, product2, customer

    // Only products in the order
    let result = db.neighbors(order.id, Direction::Outgoing, Some("contains"));
    assert_eq!(result.len(), 2);
