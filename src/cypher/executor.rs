...
impl<'a> Executor<'a> {
    ...
    /// `CALL drevo.louvain() YIELD node, community` (RFC #307 Phase 8) — Louvain
    /// community detection over the whole graph. Each node is returned with its
    /// contiguously-numbered community id (nodes sharing an id are one cluster),
    /// in ascending node order. Uses the default resolution/level config; edge
    /// weights are read from `Edge::weight`.
    fn proc_louvain(&self) -> ExecResultT<Vec<Vec<Value>>> {
        let node_ids: Vec<u64> = self.engine().all_nodes()?.iter().map(|n| n.id).collect();
        let edges: Vec<(u64, u64, f32)> = self
            .engine()
            .all_edges()?
        ...
        let graph = crate::algorithms::AdjacencyList::from_parts(node_ids, edges);
        let result =
            crate::algorithms::louvain(&graph, &crate::algorithms::LouvainConfig::default());

        let mut rows = Vec::with_capacity(result.communities.len());
        for (id, community) in result.communities {
            if let Some(node) = self.engine().get_node(id)? {
                rows.push(vec![
                    Value::Node(node_to_value(&node)),
                    Value::Integer(community as i64),
                ]);
            }
        }
        Ok(rows)
    ...
    /// `CALL drevo.scc() YIELD node, component` (RFC #307 Phase 8) — strongly
    /// connected components over the whole graph. Unlike `drevo.wcc()`, edge
    /// direction matters: two nodes share a component only if each is reachable
    /// from the other along edge direction (they lie on a common directed
    /// cycle). Each node is returned with its component id, in ascending node
    /// order; component ids are keyed by the minimum node id per component, so
    /// they are deterministic and engine-independent.
    fn proc_scc(&self) -> ExecResultT<Vec<Vec<Value>>> {
        let node_ids: Vec<u64> = self.engine().all_nodes()?.iter().map(|n| n.id).collect();
        let edges: Vec<(u64, u64, f32)> = self
            .engine()
            .all_edges()?
            ...
            .collect();
        let graph = crate::algorithms::AdjacencyList::from_parts(node_ids, edges);
        let result = crate::algorithms::scc(&graph);
    ...
    /// `CALL drevo.betweenness() YIELD node, score` (RFC #307 Phase 8) —
    /// betweenness centrality over the whole graph via Brandes' algorithm on the
    /// directed, unweighted graph (hop count; edge weights ignored). Each node
    /// is returned with the number of shortest paths through it, most-central
    /// first (ties by ascending node id).
    fn proc_betweenness(&self) -> ExecResultT<Vec<Vec<Value>>> {
        let node_ids: Vec<u64> = self.engine().all_nodes()?.iter().map(|n| n.id).collect();
        let edges: Vec<(u64, u64, f32)> = self
            .engine()
            .all_edges()?
            ...
            .collect();
        let graph = crate::algorithms::AdjacencyList::from_parts(node_ids, edges);
        let result = crate::algorithms::betweenness(&graph);
