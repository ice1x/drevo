...
- [x] `00122` **Python CI matrix on every PR** — GitHub Actions workflow `.github/workflows/python.yml`. Matrix as shipped: `{python: [3.13]} × {os: [ubuntu-latest, macos-latest, windows-latest]}` = 3 cells per PR. (The originally-spec'd 4 × 3 = 12-cell matrix was slimmed at review time: the `abi3-py310` wheel makes one binary run on every CPython 3.10+, so the middle minors mostly multiply CI spend without coverage delta — see the file preamble for full rationale.) Per-job steps:
