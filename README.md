...
- [x] `00122` **Python CI matrix on every PR** — GitHub Actions workflow `.github/workflows/python.yml`. Matrix: `{python: [3.10, 3.11, 3.12, 3.13]} × {os: [ubuntu-latest, macos-latest, windows-latest]}`. Per-job steps:
...
**Progress (2026-05-28, after task 00122):** Phase 16 Python CI matrix shipped on `feature/python-ci-matrix-00122`. `.github/workflows/python.yml` lights up the full 4 × 3 matrix declared in roadmap task `00122`: `{python: [3.10, 3.11, 3.12, 3.13]} × {os: [ubuntu-latest, macos-latest, windows-latest]}` = 12 cells per PR. Per cell: `actions/setup-python@v5` (with pip cache), `dtolnay/rust-toolchain@stable` + `Swatinem/rust-cache@v2` (keyed per matrix cell to keep warm-path `maturin develop` ≈ 30 s on Linux / 1 min on macOS+Windows vs cold ~5 min), `pip install maturin>=1.7,<2.0 pytest pytest-cov mypy ruff black`, `maturin develop --release --features="redb-backend"`, then five gates as discrete steps so the Actions UI surfaces *which* layer regressed: `pytest tests/unit/` (00118), `pytest tests/integration/` (00119), `pytest tests/e2e/` (00120), `mypy --strict python/drevo/`, `ruff check + black --check`. A final `pytest --cov=drevo --cov-report=xml` runs per cell and uploads the report as a per-cell `coverage-${os}-py${python}` artefact (7-day retention). A second job `coverage_comment` (Ubuntu, only on `pull_request`) pulls the canonical `coverage-ubuntu-latest-py3.12` artefact and posts a Markdown coverage summary to the PR via `actions/github-script@v7` — vendor-neutral so no third-party action takes a transitive dependency. Cost guards: `fail-fast: false` so the 12-cell signal stays diagnostic, `concurrency: python-matrix-${{ github.ref }}` with `cancel-in-progress: true` (distinct from `python-ci-` and `python-wheels-` so the three workflows do not cancel each other), `timeout-minutes: 30` per cell (Windows cold-build worst case), and path filters narrowed to `drevo-py/**` + `src/**` + `Cargo.toml`/`Cargo.lock` + the workflow file itself so doc-only PRs do NOT pay the matrix cost. The runner-policy test `tests/ci_self_hosted_runner_tests.rs` widens the macOS/Windows allow-list from `python-wheels.yml` only to `{python-wheels.yml, python.yml}` (`PYTHON_OS_MATRIX_WORKFLOWS` const) with the same RFC §2 wheel-layout citation: PyO3 ABI checks are platform-native (no cross-compile path), FTS UTF-8 tokenisation is locale-sensitive on Windows, redb file locking is kernel-dependent — only running on real macOS / Windows runners gives meaningful regression signal for those classes. The renamed test `macos_and_windows_runners_only_in_python_matrix_workflows` (was `..._python_wheels_workflow`) catches accidental drift. New Rust scaffolding test `tests/python_ci_matrix_tests.rs` (18 text-level cases) grep-locks the workflow's shape — top-level name + `python` token, three triggers (`pull_request: branches:[main]`, `push:`, `workflow_dispatch:`), path filters covering `drevo-py/**` + `src/**` + the workflow file, every `cp310..cp313` Python minor enumerated as `"3.10"..."3.13"`, every OS axis label enumerated, `runs-on: ${{ matrix.os }}` threading the axis, `maturin develop --release`, every tool from `pip install maturin pytest mypy ruff black`, three discrete `pytest tests/{unit,integration,e2e}/` invocations, `mypy --strict + ruff check + black --check`, `pytest-cov`/`--cov`, concurrency group `python-matrix-…` with `cancel-in-progress: true`, no PyPI publish surface, README ticking `[x] 00122` + this progress note, and `drevo-py/CHANGELOG.md` referencing `00122`. Local gate: `cargo fmt --check` (clean), `cargo clippy --workspace --all-targets -- -D warnings` (clean), `cargo nextest run` (2000 passed — baseline 1982 + 18 new), `.venv/bin/pytest tests/` (361 passed, no Python source change), `python3.13 -m black --check python/ tests/` (unchanged), `.venv/bin/ruff check python/ tests/` (clean), `.venv/bin/mypy --strict python/drevo/` (clean). The new workflow does NOT replace `python-ci.yml`: that lightweight self-hosted single-cell gate stays as the fast (~1-2 min) feedback signal alongside the matrix; both run on every Python-touching PR. 00121 (MCP introspection generator) remains in Phase 16 — blocked on Phase 15 `00090` (drevo-mcp server) which has not landed.
